...
Document versions within a protocol revision **may diverge**: the `Version:` header tracks each RFC's own editorial history, not the protocol literal, which stays `aitp/0.2` for all of them, and not the minor position, which is reserved for the protocol revision itself (see [`VERSIONING.md`](../VERSIONING.md)). RFC-AITP-0008 is at `0.2.7-draft`; RFC-AITP-0001 and RFC-AITP-0010 are at `0.2.6-draft`; RFC-AITP-0003 is at `0.2.5-draft`; RFC-AITP-0002 and RFC-AITP-0004 are at `0.2.4-draft`; and RFC-AITP-0005 is at `0.2.3-draft`, after the JCS signing-input, session-bundle placement, extensions field-table, RFC-AITP-0011 wrapper-citation, §3.1 pinned-key timestamp, `UNKNOWN_FIELD`, structural-rejection code, identity-descriptor mirror, §7.2 verification-order, and status-ladder corrections (see [`CHANGELOG.md`](../CHANGELOG.md)). RFC-AITP-0006, RFC-AITP-0007, RFC-AITP-0009 and RFC-AITP-0011 are at `0.2.1-draft` (the status-ladder normalization, their only editorial history to date). RFC-AITP-0012 and RFC-AITP-0013 keep their own `-reserved` / `-planned` version suffixes, unmoved by this normalization since their `Status:` values did not change. Check the header of the document you are reading rather than assuming a shared version.
