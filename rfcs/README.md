...
Document versions within a protocol revision **may diverge**: the `Version:` header tracks each RFC's own editorial history, not the protocol literal, which stays `aitp/0.2` for all of them. RFC-AITP-0001 and RFC-AITP-0010 are at `0.2.3-draft`, and RFC-AITP-0003 and RFC-AITP-0008 are at `0.2.2-draft`, after the JCS signing-input and session-bundle placement corrections (see [`CHANGELOG.md`](../CHANGELOG.md)); the other Draft-stage documents remain at `0.2.0-draft` (the reserved and planned placeholders keep their own suffixes). Check the header of the document you are reading rather than assuming a shared version.
