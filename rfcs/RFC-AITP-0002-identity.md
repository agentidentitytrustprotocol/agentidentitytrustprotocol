...
| `public_key` | string | REQUIRED for `pinned_key`; MUST be absent for `oidc` | base64url-encoded public key. For `pinned_key`, this is the key the verifier looks up in its `pinned_keys` configuration (§3.2). For `oidc`, the agent's public key is already encoded in the verifying-peer envelope's `sender.agent_id` (the AID-identifier component) — there is no second copy in the descriptor, and including one would create an ambiguity over which key the JWT's `cnf.jkt` is meant to bind. The Manifest schema enforces this absent-for-oidc rule (`schemas/json/aitp-manifest.schema.json` `IdentityHint.if/then/else`); v0.1 verifiers MUST reject an OIDC `identity` descriptor that carries `public_key`. |
