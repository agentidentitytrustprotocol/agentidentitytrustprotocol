...
4. **Mandatory.** Every AITP agent MUST publish a Manifest. There is no service-only profile in v0.2; every agent participates in A2A.
...
| `proof_of_possession.signature` | string | `base64url(sign(agent_private_key, sha256(base64url_decode(challenge))))`. Exactly 86 chars unpadded base64url. The hash input MUST be the 16 raw bytes obtained by base64url-decoding `challenge` (not the base64url string itself). This follows the same convention as RFC-AITP-0004 §3 and RFC-AITP-0005 §6.1: every PoP signing input in AITP v0.2 hashes decoded bytes, never encoded strings. See the unified PoP signing-input convention in [RFC-AITP-0001 §5.4.2](RFC-AITP-0001-core.md#542-pop-signing-input-convention). |
...
- A non-2xx response (HTTP 404, 5xx, DNS failure, TLS failure) means the Manifest could not be retrieved. The fetching peer MUST treat the result as `MANIFEST_NOT_FOUND` (§10) for any downstream decision keyed on Manifest absence vs. expiry vs. signature failure. `MANIFEST_NOT_FOUND` is retryable per the registry — peers MAY back off and retry per their fetch policy.
...
6. **Identity-type and trust-anchor compatibility** — The fetching peer MUST screen the published Manifest against its own identity. Two cases, with **distinct error codes**:
   ...
   These two codes are **not interchangeable**: `INCOMPATIBLE_TRUST_ANCHORS` signals an OIDC trust-anchor overlap failure, while `INCOMPATIBLE_IDENTITY_TYPE` signals that the target Manifest does not accept the fetching peer's identity *type*. Implementations MUST emit the more specific code when the failure is identity-type incompatibility — collapsing both into `INCOMPATIBLE_TRUST_ANCHORS` hides a configuration error (publisher restricted to OIDC; fetcher is pinned-key) behind a misleading "issuer not trusted" code. If neither check passes, the handshake MUST NOT be initiated and the peer SHOULD log the appropriate code per the matrix above.
...
3. Notify known peer agents of the new AID through out-of-band channels (the protocol does not provide a normative push mechanism in v0.2; see [RFC-AITP-0009 §1.10](RFC-AITP-0009-security.md#110-key-compromise)).
...
There is no AID-level revocation mechanism in v0.2. The AID is derived from the public key, so a new key produces a new AID; trust in the old AID is terminated by setting the old Manifest's `expires_at` to a value in the past (or letting it expire naturally) and by revoking every TCT issued under it. Peers that learn of a key compromise out-of-band MUST NOT continue to trust the old AID even if its cached Manifest has not yet expired.

---

## 9. Manifest delivery in v0.2

The normative delivery mechanism is the well-known HTTPS endpoint defined in §4. Inline delivery during the Mutual Handshake (`payload.manifest`) is the second supported path. v0.2 conformance does not require any RPC service for Manifest fetching.
