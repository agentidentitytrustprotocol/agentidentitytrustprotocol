...
| `del-*` | `verify_delegation_token` | Single-hop delegation verification (RFC-AITP-0006 §4). Inputs may include a `revocation_snapshots` array of `{issuer_aid, snapshot}` records — the same shape `del-mh-*` uses — that the runner MUST populate the deny list from before checking `voucher.src_jti` (RFC-AITP-0006 §4 step 7). Unlike `del-mh-*`, core §4 names exactly this one lookup: no per-hop sweep (there are no hops) and no independent voucher revocation handle (RFC-AITP-0008 §1.1). |
