...
Fixtures that exercise stateful behavior (e.g. replay detection across two sends) use a `sequence` array inside `input` instead of a single `expected` block. Each entry is one step:

```json
{
  "input": {
    "sequence": [
      { "step": 1, "message_id": "...", "expected": { "outcome": "success" } },
      { "step": 2, "message_id": "...", "expected": { "outcome": "failure", "error_code": "REPLAY_DETECTED" } }
    ]
  }
}
```

The runner MUST execute each step in order against the same implementation instance and assert the per-step `expected` outcome. `mh-001` is the only v0.1 fixture using this form; future replay or session-state fixtures SHOULD follow the same shape.
